    ...
    ui(new Ui::VentanaPrincipal)
{
    ...
    for (int i = 0; i < 8; i++){
        ...
        for (int j = 0; j < 8; j++){
            ...
            for (int k = 0 ; k < 6 ; k++){//[arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
            ...
            hiloCasilla->setInfo(&(arbolesEnGranja),i,j,&posXGranjero, &posYGranjero, &probabilidadAparicionCuervo, &cantidadCuervosAparecen, &cantidadTardanAparecerCuervos, &frutosComeCuervo, &segundosCuervoComeFrutos, &probabilidadAparicionOveja, &cantidadOvejasAparecen, &cantidadTardanAparecerOvejas, &frutosComeOveja, &segundosOvejaComeFrutos, &probabilidadAparicionPlaga, &cantidadTardaAparecerPlaga, &cantidadSegundosDestruyePlaga, &crecimientoABB, &frutosCosechaABB, &cantSegCosechaABB,&crecimientoHeap, &frutosCosechaHeap, &cantSegCosechaHeap, &crecimientoAVL, &frutosCosechaAVL, &cantSegCosechaAVL, &crecimientoBonus, &frutosCosechaBonus, &cantSegCosechaBonus, (int)hiloCasilla->semilla);
            connect (hiloCasilla, SIGNAL(aumentarCantidadFrutos()), this, SLOT(aumentarCantidadFrutos()));
            connect (hiloCasilla, SIGNAL(anadirCuervo()), this, SLOT(anadirCuervo()));
            connect (hiloCasilla, SIGNAL(anadirOveja()), this, SLOT(anadirOveja()));
            connect (hiloCasilla, SIGNAL(anadirPlaga()), this, SLOT(anadirPlaga()));
            connect (hiloCasilla, SIGNAL(actualizarMercado()), this, SLOT(actualizarMercado()));
            connect (hiloCasilla, SIGNAL(espantarPlagas()), this, SLOT(espantarPlagas()));
            //hiloCasilla->start(); Esto hace que se caiga el programa :D Por eso se hace más abajo
        ...
        matrizBoolColocables.push_back(vectorParaMatrizBool);//[arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    }

    for(int i = 0; i < matrizHilosCasilla.length();i++){
        for (int j = 0; j < matrizHilosCasilla[i].length(); j++){
            //ARRIBA IZQUIERDA ABAJO DERECHA
            switch (i) {
            case 0://No tiene izquierda
                switch (j) {
                case 0://No tiene arriba
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(nullptr, nullptr, &(matrizBoolColocables[i][j+1][4]), &(matrizBoolColocables[i+1][j][4]));
                    break;
                case 7://No tiene abajo
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), nullptr, nullptr, &(matrizBoolColocables[i+1][j][4]));
                    break;
                default:
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), nullptr, &(matrizBoolColocables[i][j+1][4]), &(matrizBoolColocables[i+1][j][4]));
                    break;
                }
                break;
            case 7://No tiene Derecha
                switch (j) {
                case 0://No tiene arriba
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(nullptr, &(matrizBoolColocables[i-1][j][4]), &(matrizBoolColocables[i][j+1][4]), nullptr);
                    break;
                case 7://No tiene abajo
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), &(matrizBoolColocables[i-1][j][4]), nullptr, nullptr);
                    break;
                default:
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), &(matrizBoolColocables[i-1][j][4]), &(matrizBoolColocables[i][j+1][4]), nullptr);
                    break;
                }
                break;
            default:
                switch (j) {
                case 0://No tiene arriba
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(nullptr, &(matrizBoolColocables[i-1][j][4]), &(matrizBoolColocables[i][j+1][4]), &(matrizBoolColocables[i+1][j][4]));
                    break;
                case 7://No tiene abajo
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), &(matrizBoolColocables[i-1][j][4]), nullptr, &(matrizBoolColocables[i+1][j][4]));
                    break;
                default:
                    matrizHilosCasilla[i][j]->setPunterosBooleanos(&(matrizBoolColocables[i][j-1][4]), &(matrizBoolColocables[i-1][j][4]), &(matrizBoolColocables[i][j+1][4]), &(matrizBoolColocables[i+1][j][4]));
                    break;
                }
                break;
            }
            matrizHilosCasilla[i][j]->start();
        }
...
void VentanaPrincipal::actualizarUnaCeldaXY(short posX, short posY){
    //Ultima posicion de matriz bool colocables [arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    QPixmap pixMap;
    QPixmap pixMap2;
    QPixmap pixMap3;
    QPixmap pixMap4;
    switch(arbolesEnGranja[posX][posY][0]){
        case 0://No hay arbol
            matrizLabelsArbolesMisc[posX][posY]->clear();
            break;
        case 1://ABB
            if(matrizBoolColocables[posX][posY][5]){//Parasito
                pixMap.load(":/arbol/binario_ordenado/Images/deadtrees/parasitoABB.png");
                pixMap = pixMap.scaled(60, 60, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                matrizLabelsArbolesMisc[posX][posY]->setGeometry(210+(posX*80),40+(posY*80),60,60);
                matrizLabelsArbolesMisc[posX][posY]->setPixmap(pixMap);
            }
        ...
        case 2://HEAP
            if(matrizBoolColocables[posX][posY][5]){//Parasito
                pixMap.load(":/arbol/heap/Images/deadtrees/parasitoHeap.png");
                pixMap = pixMap.scaled(61, 71, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                matrizLabelsArbolesMisc[posX][posY]->setGeometry(220+(posX*80),30+(posY*80),61,71);
                matrizLabelsArbolesMisc[posX][posY]->setPixmap(pixMap);
            }
        ...
        case 3://AVL
            if(matrizBoolColocables[posX][posY][5]){//Parasito
                pixMap.load(":/arbol/avl/Images/deadtrees/parasitoAVL.png");
                pixMap = pixMap.scaled(61, 71, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                matrizLabelsArbolesMisc[posX][posY]->setGeometry(220+(posX*80),30+(posY*80),61,71);
                matrizLabelsArbolesMisc[posX][posY]->setPixmap(pixMap);
            }
        ...
        case 4://BONUS
            if(matrizBoolColocables[posX][posY][5]){//Parasito
                pixMap.load(":/arbol/bonus/Images/deadtrees/parasitoBonus.png");
                pixMap = pixMap.scaled(61, 95, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                matrizLabelsArbolesMisc[posX][posY]->setGeometry(230+(posX*80),20+(posY*80),61,95);
                matrizLabelsArbolesMisc[posX][posY]->setPixmap(pixMap);
            }
    ...
    if (matrizBoolColocables[posX][posY][3]){
        pixMap4.load(":/oveja/Images/OvejaObesa.png");
        pixMap4 = pixMap4.scaled(61, 51, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        matrizLabelsOveja[posX][posY]->setGeometry(190+(posX*80),40+(posY*80),61,51);
        matrizLabelsOveja[posX][posY]->setPixmap(pixMap4);
    }
    else if (matrizBoolColocables[posX][posY][2]){
        pixMap3.load(":/oveja/Images/OVEJA.png");
        pixMap3 = pixMap3.scaled(31, 31, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        matrizLabelsOveja[posX][posY]->setGeometry(190+(posX*80),70+(posY*80),31,31);
        matrizLabelsOveja[posX][posY]->setPixmap(pixMap3);
        ...
        (*(matrizLabelsOveja[posX][posY])).clear();
    }

    if (matrizBoolColocables[posX][posY][1]){
        ...
        */
        ...
        (*(matrizLabelsCuervo[posX][posY])).clear();
    }
}
...
void VentanaPrincipal::actualizarMercado(){

    int posXSender = ((HiloCasillaTablero*)QObject::sender())->posXHilo;
    int posYSender = ((HiloCasillaTablero*)QObject::sender())->posYHilo;
    if(interfazMercado != nullptr && cronometroTiempoRestante->remainingTime() > 3){
        //No está oculta
        interfazMercado->actualizarArbol(posXSender,posYSender);
    }
}

void VentanaPrincipal::anadirCuervo(){
    int posXSender = ((HiloCasillaTablero*)QObject::sender())->posXHilo;
    int posYSender = ((HiloCasillaTablero*)QObject::sender())->posYHilo;
    //La matrizBoolColocables va a ser 8x8x5. En 5 va [arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    if(!matrizBoolColocables[posXSender][posYSender][1]){
        //No hay cuervo por lo que inserto uno visual
        matrizBoolColocables[posXSender][posYSender][1] = true;
        actualizarUnaCeldaXY(posXSender,posYSender);
    }
}
void VentanaPrincipal::anadirOveja(){
    int posXSender = ((HiloCasillaTablero*)QObject::sender())->posXHilo;
    int posYSender = ((HiloCasillaTablero*)QObject::sender())->posYHilo;
    //La matrizBoolColocables va a ser 8x8x5. En 5 va [arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    if(!matrizBoolColocables[posXSender][posYSender][2]){
        //No hay oveja por lo que inserto una visual
        matrizBoolColocables[posXSender][posYSender][2] = true;
        actualizarUnaCeldaXY(posXSender,posYSender);
    }
}
void VentanaPrincipal::anadirPlaga(){
    int posXSender = ((HiloCasillaTablero*)QObject::sender())->posXHilo;
    int posYSender = ((HiloCasillaTablero*)QObject::sender())->posYHilo;
    //La matrizBoolColocables va a ser 8x8x5. En 5 va [arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    if(!matrizBoolColocables[posXSender][posYSender][5]){
        //No hay cuervo por lo que inserto uno visual
        matrizBoolColocables[posXSender][posYSender][5] = true;
        actualizarUnaCeldaXY(posXSender,posYSender);
    }
}

void VentanaPrincipal::espantarPlagas(){
    int posXSender = ((HiloCasillaTablero*)QObject::sender())->posXHilo;
    int posYSender = ((HiloCasillaTablero*)QObject::sender())->posYHilo;
    //La matrizBoolColocables va a ser 8x8x5. En 5 va [arbol][cuervo][oveja][ovejaObesa][espanta][plaga]
    matrizBoolColocables[posXSender][posYSender][1] = false;
    matrizBoolColocables[posXSender][posYSender][2] = false;
    matrizBoolColocables[posXSender][posYSender][3] = false;
    matrizBoolColocables[posXSender][posYSender][5] = false;
    actualizarUnaCeldaXY(posXSender, posYSender);
